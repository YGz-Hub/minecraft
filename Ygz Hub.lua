...
local Config = {
    Enabled = true,
    AttackDistance = 80,
    AttackSpeed = 0.0000000000000000000000000000000000000001,
    MaxCombo = 50,
    ComboReset = 0.000001,
    HitLimbs = {"Head", "HumanoidRootPart", "Torso", "RightHand", "LeftHand"}
}
    ...
    for _, enemy in pairs(Workspace.Enemies:GetChildren()) do
        ...
        if hum and root and hum.Health > 0 then
            ...
            if dist <= Config.AttackDistance then
                targets[#targets + 1] = {enemy, root, dist}
            end
        end
    end
    ...
    for _, player in pairs(Workspace.Characters:GetChildren()) do
        if player ~= char then
            ...
            if hum and root and hum.Health > 0 then
                local dist = (pos - root.Position).Magnitude
                if dist <= Config.AttackDistance then
                    targets[#targets + 1] = {player, root, dist}
                end
            end
        end
    end
...
function FA:GetCombo()
    local now = tick()
    if (now - self.LastCombo) > Config.ComboReset then
        self.ComboCount = 0
    end
    self.ComboCount = (self.ComboCount % Config.MaxCombo) + 1
    self.LastCombo = now
    return self.ComboCount
end

function FA:CanAttack()
    local char = Player.Character
    ...
    return (tick() - self.LastAttack) >= Config.AttackSpeed
end

function FA:ExecuteAttack()
    if not Config.Enabled or not self:CanAttack() then return end
    ...
    if tool.ToolTip == "Gun" then
        self:HandleGun(targets[1][2])
    elseif tool.ToolTip == "Blox Fruit" and tool:FindFirstChild("LeftClickRemote") then
        self:HandleFruit(targets, combo)
    else
        self:HandleMelee(targets)
    end
end

function FA:HandleMelee(targets)
    local hitData = {}
    local mainTarget = nil
    
    for i, target in pairs(targets) do
        local limb = target[1]:FindFirstChild(Config.HitLimbs[math.random(#Config.HitLimbs)])
        if limb then
            if not mainTarget then
                mainTarget = limb
            end
            hitData[#hitData + 1] = {target[1], limb}
        end
    end
    
    if mainTarget then
        RegisterAttack:FireServer(Config.AttackSpeed)
        RegisterHit:FireServer(mainTarget, hitData)
    end
end

function FA:HandleFruit(targets, combo)
    if #targets == 0 then return end
    local char = Player.Character
    local tool = char:FindFirstChildOfClass("Tool")
    local direction = (targets[1][2].Position - char.HumanoidRootPart.Position).Unit
    tool.LeftClickRemote:FireServer(direction, combo)
end

function FA:HandleGun(target)
    local char = Player.Character
    local tool = char:FindFirstChildOfClass("Tool")
    if not tool or tool.ToolTip ~= "Gun" then return end
    
    local cooldown = tool:FindFirstChild("Cooldown")
    if cooldown and (tick() - (self.Cache[tool] or 0)) < cooldown.Value then return end
    
    self.Cache[tool] = tick()
...
function Library:CreateWindow(info)
	...
	if type(Icon) == 'string' and not Icon:find('rbxassetid://') then
		ToggleIcon.Image = "rbxassetid://".. Icon
	elseif type(Icon) == 'number' then
		ToggleIcon.Image = "rbxassetid://".. Icon
	else
